...
export async function POST(req: Request) {
  ...
  const rl = checkRateLimit(`preview:${ip}`, { limit: 10, windowMs: 24 * 60 * 60 * 1000 });
  if (!rl.allowed) {
  ...
  return NextResponse.json({ ok: true, data: generatePreview(input) }, { headers: rateLimitHeaders(rl) });
}
