...
const buildPaidReportPrompt = (params: { input: FortuneInput; productCode: ProductCode; targetModel?: string; charTarget?: number }) => {
  ...
  const system =
    ...
    "- 문체: 존댓말, 따뜻하고 친근한 톤. 친구가 쉽게 설명해주듯이 작성\n" +
    "- 핵심 원칙: 사주 전문용어(일간, 천간, 지지, 비견, 식신 등)를 그대로 쓰지 마세요. 반드시 일상 언어로 풀어서 설명하세요.\n" +
    "  예시: [일간 기토가 월간 경금에 극을 받는다] -> [당신의 타고난 성향은 부드럽고 포용적인데, 환경에서 오는 압박이나 도전이 있을 수 있어요]\n" +
    "  예시: [비견이 많다] -> [자기주장이 강하고 독립심이 센 편이에요]\n" +
    "- 목표: 읽는 사람이 공감하며 '그래서 뭘 하면 되지?'가 남도록 구체적 행동 제안 포함\n" +
  ...
  const user =
    ...
    "}\n\n" +
    "작성 규칙:\n" +
    "- 전문 용어 없이 일상 언어로 작성. 사주 개념을 언급할 때는 반드시 쉽게 풀어서 설명하세요.\n" +
    "- 각 섹션은 최소 6~10문장 이상, 구체적인 예시와 실생활 행동 팁을 포함하세요.\n" +
...
export const generateChunkedReport = async (params: {
  ...
  for (const sec of SECTION_KEYS) {
    const userPrompt =
      ...
      `전문 용어 없이 일상 언어로, 구체적 예시와 실생활 행동 팁을 포함해 주세요.\n` +
      `'~할 수 있어요', '~하는 편이에요' 같은 부드러운 표현을 사용하세요.\n` +
      `본문 텍스트만 출력하세요.`;
